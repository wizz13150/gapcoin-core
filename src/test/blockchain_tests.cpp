...
CBlockIndex* CreateBlockIndexWithDifficulty(uint64_t nDifficulty)
{
    CBlockIndex* block_index = new CBlockIndex();
    block_index->nHeight = 46367;
    block_index->nTime = 1269211443;
    block_index->nDifficulty = nDifficulty;
    return block_index;
}

CChain CreateChainWithDifficulty(uint64_t nDifficulty)
{
    CBlockIndex* block_index = CreateBlockIndexWithDifficulty(nDifficulty);
    CChain chain;
 ...
 */
void TestDifficulty(uint64_t nDifficulty, double expected_difficulty)
{
    CBlockIndex* block_index = CreateBlockIndexWithDifficulty(nDifficulty);
...
{
    CChain chain = CreateChainWithDifficulty(0x15690d0d64b3770);

    double difficulty = GetDifficulty(chain, nullptr);
    delete chain.Tip();

    double expected_difficulty = 342.565687;
...
BOOST_AUTO_TEST_CASE(get_difficulty_for_block_index_overrides_tip)
{
     ...
     */
    CBlockIndex* override_block_index = CreateBlockIndexWithDifficulty(0x140000000000000);

    double difficulty = GetDifficulty(chain, override_block_index);
    delete chain.Tip();
    delete override_block_index;

    RejectDifficultyMismatch(difficulty, 320.0 /*5913134931067755359633408.0*/);
}
